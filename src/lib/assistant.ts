...
export function generateAssistantAnswer(message: string, orders: Order[], expenses: Expense[] = []) {
  ...
  if (includesAny(text, ["календар", "дата", "расписан"])) {
    return "Календарь теперь находится на главной: выберите дату в мини-календаре, и справа появятся события выбранного дня. Заказ можно открыть из списка событий или из ближайших заказов.";
  }
